...
statsMod::statsMod(const std::string &filePath){
    ...
    // Trim whitespaces from the beginning and end of each column name.
...
// Copy constructor.
statsMod::statsMod(const statsMod& other) : data(other.data), columns(other.columns), numCols(other.numCols), catCols(other.catCols), ncol(other.ncol), nrow(other.nrow){}


// Copy assignment operator
statsMod& statsMod::operator=(const statsMod& other) {
    if (this != &other) {
        data = other.data;
...
std::map<std::string, uint> statsMod::freq(const std::string &var) const{
    ...
    for(const auto &val : data[var]){
        ...
        if(val.has_value()){
            ...
            std::string varVal = std::get<std::string>(varValue);
...
// Method that removes every row that possesses any NA value.
...
// Method that summarizes the the data-set calculating various statistics on it.
